...
namespace LoggerClassProject.Persistance
{
    public class DbConnectionFactory : IConnectionFactory
    {
        ...
        public DbConnectionFactory(string connectionName)
        {
            ...
            if (conStr == null)
                throw new ConfigurationErrorsException(string.Format("ConnectionString named '{0}' not founded in app/web.config.", connectionName));
        ...
        public IDbConnection Create()
        {
            ...
            if (connection == null)
                throw new ConfigurationErrorsException(string.Format("Failed to create a connection using ConnectionString named '{0}' in app/web.config.", _name));
